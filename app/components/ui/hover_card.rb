...
module Components
  module Ui
    class HoverCard < Components::Base
      def initialize(open: nil, placement: nil, offset: nil, delay: nil, **attrs)
        @attrs = attrs
        @attrs[:data] ||= {}
        @attrs[:data][:controller] = "ui--hover-card" unless @attrs[:data][:controller]

        if !open.nil? && !@attrs[:data].key?(:ui__hover_card_open_value) && !@attrs[:data].key?("ui__hover_card_open_value")
          @attrs[:data][:ui__hover_card_open_value] = open
        elsif !@attrs[:data].key?(:ui__hover_card_open_value) && !@attrs[:data].key?("ui__hover_card_open_value")
          @attrs[:data][:ui__hover_card_open_value] = false
        end

        if !placement.nil? && !@attrs[:data].key?(:ui__hover_card_placement_value) && !@attrs[:data].key?("ui__hover_card_placement_value")
          @attrs[:data][:ui__hover_card_placement_value] = placement
        end

        if !offset.nil? && !@attrs[:data].key?(:ui__hover_card_offset_value) && !@attrs[:data].key?("ui__hover_card_offset_value")
          @attrs[:data][:ui__hover_card_offset_value] = offset
        end

        if !delay.nil? && !@attrs[:data].key?(:ui__hover_card_delay_value) && !@attrs[:data].key?("ui__hover_card_delay_value")
          @attrs[:data][:ui__hover_card_delay_value] = delay
        end
      end
      ...
      def trigger(**attrs, &block)
        render Trigger.new(**with_required_data(
          ...
          data: {
            ...
            action: "mouseenter->ui--hover-card#show mouseleave->ui--hover-card#hide focusin->ui--hover-card#show focusout->ui--hover-card#hide"
          }
        )), &block
      end
