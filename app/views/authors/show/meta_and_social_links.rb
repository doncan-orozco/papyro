        ...
        div(class: "mt-12 max-w-xl border-y border-border/40") do
          if @presenter.location.present?
            div(class: "flex items-center justify-between gap-4 border-b border-border/40 py-3") do
              span(class: "text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground/80") { t("authors.show.meta.location") }
              div(class: "inline-flex items-center gap-2 text-sm font-medium text-foreground") do
                render Components::Ui::Icon.new(:map_pin, class: "size-4 shrink-0 text-muted-foreground")
          ...
          if @presenter.website_url.present?
            div(class: "flex items-center justify-between gap-4 border-b border-border/40 py-3") do
              span(class: "text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground/80") { t("authors.show.meta.website") }
              link_to(
                @presenter.website_url,
                target: "_blank",
                rel: "noopener noreferrer",
                class: "inline-flex items-center gap-2 text-sm font-medium text-foreground transition-colors hover:text-rose-600 dark:hover:text-rose-500"
              ) do
          ...
          if @presenter.x_handle.present?
            div(class: "flex items-center justify-between gap-4 border-b border-border/40 py-3") do
              span(class: "text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground/80") { t("authors.show.meta.x") }
              link_to(
          ...
          if @presenter.linkedin_handle.present?
            div(class: "flex items-center justify-between gap-4 py-3") do
              span(class: "text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground/80") { t("authors.show.meta.linkedin") }
              link_to(
